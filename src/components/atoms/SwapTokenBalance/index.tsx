...
export const SwapTokenBalance = ({token,amoutSwapTokenSetter,disabled=false,amoutSwapToken=1}) => {
  ...
  return (
    <SwapTokenBalanceStyled>
      {!disabled && <InputStyled type='number' disabled={disabled} placeholder="0.0001" defaultValue={max} onChange={(e)=>{amoutSwapTokenSetter(e.target.value)}}/>}
      {disabled && <InputStyled type='number' value={amoutSwapToken} onChange={(e)=>{amoutSwapTokenSetter(e.target.value)}}/>}
